...
class Logger(object):
    ...
    def __enter__(self):
        ...
        fieldnames = ['episode', 'reward', 'winrate', 'knock rate', 'moves avg', 'unsafe avg', 'org rewards']
    ...
    def log_performance(self, episode, stats):
        ''' Log a point in the curve
        Args:
            episode (int): the episode of the current point
            reward (float): the reward of the current point
        '''
        self.writer.writerow({'episode': episode, 'reward': stats[0], 'winrate': stats[1], 'knock rate': stats[2], 'moves avg': stats[3], 'unsafe avg': stats[4], 'org rewards': stats[5]})
        print('')
        self.log('----------------------------------------')
        self.log('  episode      |  ' + str(episode))
        self.log('  reward       |  ' + str(stats[0]))
        self.log('  winrate      |  ' + str(stats[1]))
        self.log('  knock rate   |  ' + str(stats[2]))
        self.log('  moves avg    |  ' + str(stats[3]))
        self.log('  unsafe avg   |  ' + str(stats[4]))
        self.log('  org rewards  |  ' + str(stats[5]))
        self.log('----------------------------------------')
